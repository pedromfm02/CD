...
class FingerTable:
    ...
    def __init__(self, node_id, node_addr, m_bits=10):
        ...
        for m in range(self.m_bits):
            self.table[m+1] = ((self.node_id+2**m) % (2**self.m_bits),0)
        

    def fill(self, node_id, node_addr):
        """ Fill all entries of finger_table with node_id, node_addr."""
        value = (node_id,node_addr)
    ...
    def find(self, identification):
        ...
        for m in range(self.m_bits, 0, -1):
            if (identification != self.table[m][0] and contains(self.node_id, identification, self.table[m][0])):
                closest_node_addr = self.table[m][1]
                break
        if closest_node_addr is None:
            closest_node_addr = self.table[1][1]

        return closest_node_addr

    def refresh(self):
        """ Retrieve finger table entries requiring refresh."""
        idx_list = []
        for m in range(self.m_bits):
            idx_list.append((m+1,(self.node_id+2**m) % (2**self.m_bits),self.table[m+1][1]))
        return idx_list
    ...
    def as_list(self):
        ...
        for key in self.table:
            lst.append((self.table[key][0],self.table[key][1]))
        return lst
...
class DHTNode(threading.Thread):
    ...
    def __init__(self, address, dht_address=None, timeout=3):
        ...
        self.finger_table = FingerTable(self.identification,self.addr)    #TODO create finger_table(DONE)
    ...
    def node_join(self, args):
        ...
        if self.identification == self.successor_id:  # I'm the only node in the DHT
            ...
            self.finger_table.update(1,identification,addr) #TODO update finger table(DONE)
    ...
    def get_successor(self, args): 
        ...
        if self.predecessor_id is None or contains(self.predecessor_id,self.identification,identification):
            self.send(addr, {"method": "SUCCESSOR_REP", "args": {"req_id": identification,"successor_id": self.identification, "successor_addr": self.addr}})
        elif contains(self.identification,self.successor_id,identification):
            ...
            # 'pesquisa' remota
            #self.send(self.successor_addr,{"method": "SUCCESSOR", 'args': {"id": identification, "from": addr}})
            
            # 'pesquisa' finger_table
    ...
    def stabilize(self, from_id, addr):
        ...
        if from_id is not None and contains(self.identification, self.successor_id, from_id):
            ...
            self.successor_addr = addr
            self.finger_table.update(1,self.successor_id,self.successor_addr)#TODO update finger table(DONE)

        # notify successor of our existence, so it can update its predecessor record
        args = {"predecessor_id": self.identification, "predecessor_addr": self.addr}
        self.send(self.successor_addr, {"method": "NOTIFY", "args": args})

        refreshed_finger_table = self.finger_table.refresh() # TODO refresh finger_table(DONE)
        for idx in refreshed_finger_table:
            self.send(self.successor_addr,{"method": "SUCCESSOR", 'args': {"id": idx[1], "from": self.addr}})
    ...
    def put(self, key, value, address):  
        ...
        if self.predecessor_id is not None and contains(self.predecessor_id,self.identification,key_hash):
        ...
        while not self.inside_dht:
            ...
            if payload is not None:
                ...
                if output["method"] == "JOIN_REP":
                    ...
                    self.finger_table.fill(self.successor_id,self.successor_addr)#TODO fill finger table(DONE)
                    self.inside_dht = True
                    self.logger.info(self)
        ...
        while not self.done:
            ...
            if payload is not None:
                ...
                if output["method"] == "JOIN_REQ":
                    ...
                    self.logger.debug("Finger_Table: %s",self.finger_table)
